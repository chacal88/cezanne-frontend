...
describe("candidate database routing helpers", () => {
  ...
  it("degrades invalid URL state to stable defaults while preserving valid values", () => {
    expect(
      parseCandidateDatabaseSearchFromUrl(
        "?query=alex&page=-1&sort=invalid&order=sideways&tags=one,,two",
      ),
    ).toEqual({
      query: "alex",
      page: 1,
  ...
  it("builds canonical database paths and database-origin detail handoff URLs", () => {
    expect(
      buildCandidateDatabasePath({
        ...
        tags: ["senior"],
        advancedMode: true,
        advancedQueryId: "query-1",
        advancedQueryState: "unsupported",
      }),
    ).toBe(
    ...
    expect(
      buildCandidateDatabaseDetailPath("candidate-123", {
        query: "alex",
        page: 2,
      }),
    ).toBe(
    ...
    expect(
      ...
      "/candidate/candidate-123/13/15?entry=database&returnTo=%2Fcandidates-database%3Fquery%3Dalex%26page%3D2",
    );
  });
  ...
  it("sanitizes database-origin return targets to the canonical route", () => {
    expect(
      sanitizeCandidateDatabaseReturnTarget(
        "https://example.com/candidates-database",
      ),
    ).toBe("/candidates-database");
    expect(sanitizeCandidateDatabaseReturnTarget("/jobs/open")).toBe(
      "/candidates-database",
    );
  });
});
