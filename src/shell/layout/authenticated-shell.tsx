...
import { getSidebarNavigationEntries, getVisibleAccountNavigation, getVisiblePlatformNavigation, type SidebarNavigationLink } from '../navigation/nav-registry';
...
function useShellLabel() {
  ...
  return (key: string) => t(key, { defaultValue: shellLabelFallbacks[key] ?? key });
}

function materializePath(link: Pick<SidebarNavigationLink, 'to' | 'params'>) {
  return Object.entries(link.params ?? {}).reduce((path, [key, value]) => path.replace(`$${key}`, value), link.to);
}

function isLinkActive(pathname: string, link: Pick<SidebarNavigationLink, 'to' | 'params'>) {
...
function NavLinkItem({ item, collapsed, onNavigate }: { item: SidebarNavigationLink; collapsed: boolean; onNavigate?: () => void }) {
  ...
  return (
    <Link
      ...
      onClick={onNavigate}
    >
      {item.icon ? <i className={`fas fa-${item.icon} recruit-shell__nav-icon`} aria-hidden="true" /> : null}
      {!collapsed ? <span className="recruit-shell__nav-label">{label(item.labelKey)}</span> : null}
      {item.badgeKey && !collapsed ? <span className="recruit-shell__nav-badge">{label(item.badgeKey)}</span> : null}
    </Link>
  );
}

function NavGroupItem({ group, collapsed, pathname }: { group: SidebarGroup; collapsed: boolean; pathname: string }) {
  const label = useShellLabel();
  ...
  return (
    <div className={`recruit-shell__nav-group${open ? ' is-open' : ''}${active ? ' is-active' : ''}`}>
      <div className="recruit-shell__nav-group-row">
        <Link
          ...
          activeProps={{ className: 'recruit-shell__nav-item recruit-shell__nav-group-trigger is-active' }}
          title={label(group.labelKey)}
        >
          <i className={`fas fa-${group.icon} recruit-shell__nav-icon`} aria-hidden="true" />
          {!collapsed ? <span className="recruit-shell__nav-label">{label(group.labelKey)}</span> : null}
        </Link>
        {!collapsed ? (
          <button
            type="button"
            className="recruit-shell__nav-group-toggle"
            title={open ? `Collapse ${label(group.labelKey)}` : `Expand ${label(group.labelKey)}`}
            aria-label={open ? `Collapse ${label(group.labelKey)}` : `Expand ${label(group.labelKey)}`}
            aria-expanded={open}
            onClick={() => setOpen((value) => !value)}
          >
            <i className={`fas fa-caret-${open ? 'up' : 'down'} recruit-shell__nav-caret`} aria-hidden="true" />
          </button>
        ) : null}
      </div>
      {open ? (
        <div className="recruit-shell__submenu" role="group" aria-label={label(group.labelKey)}>
...
export function AuthenticatedShell({ children }: PropsWithChildren) {
  ...
  const shellClass = `recruit-shell${collapsed ? ' recruit-shell--collapsed' : ''}`;
  const profileLink = accountItems.find((link) => link.id === 'user-profile');
  const organizationProfileLink = accountItems.find((link) => link.id === 'hiring-company-profile' || link.id === 'recruitment-agency-profile');
        ...
        <nav className="recruit-shell__nav" aria-label={t('authenticatedShell.title')}>
          {platformSidebarGroups.length ? (
            <div className="recruit-shell__nav-section">
              {!collapsed ? <div className="recruit-shell__nav-section-label">{t('platformNavigation.label')}</div> : null}
              {platformSidebarGroups.map((group) => (
                <NavGroupItem key={group.id} group={group} collapsed={collapsed} pathname={pathname} />
              ))}
            </div>
          ) : null}
          {navEntries.map((entry) => {
            if (entry.kind === 'link') return <NavLinkItem key={entry.id} item={entry} collapsed={collapsed} />;
            return <NavGroupItem key={entry.id} group={entry} collapsed={collapsed} pathname={pathname} />;
          })}
        </nav>
      </aside>
      <div className="recruit-shell__main">
        <header className="recruit-shell__topbar">
          ...
          <div className="recruit-shell__topbar-right">
            <a className="recruit-shell__help" href="#help">
              <i className="fas fa-question-circle" aria-hidden="true" /> Get help
            </a>
            <Link className="recruit-shell__notif" to="/notifications" aria-label="Notifications">
                ...
                <div className="recruit-shell__user-menu" role="menu">
                  <div className="recruit-shell__user-menu-header">
                    ...
                    <div className="recruit-shell__user-menu-name">
                      <strong>{accessContext.isSysAdmin ? 'SysAdmin' : 'Recruiter'}</strong>
                      <span>{accessContext.organizationType === 'ra' ? 'Recruitment agency' : accessContext.organizationType === 'hc' ? 'Hiring company' : 'Platform'}</span>
                    </div>
                  </div>
                  {organizationProfileLink ? (
                    <Link className="recruit-shell__user-menu-link" to={organizationProfileLink.to as never} onClick={() => setUserMenuOpen(false)}>
                      {t(organizationProfileLink.labelKey)}
                    </Link>
                  ) : null}
                  <div className="recruit-shell__user-menu-actions">
                    {profileLink ? (
                      <Link className="recruit-shell__user-menu-profile" to={profileLink.to as never} onClick={() => setUserMenuOpen(false)}>
                        <i className="fas fa-user" aria-hidden="true" /> Profile
                      </Link>
                    ) : null}
                    {logoutLink ? (
                      <Link className="recruit-shell__user-menu-signout" to={logoutLink.to as never} onClick={() => setUserMenuOpen(false)}>
                        Sign out
                      </Link>
                    ) : null}
                  </div>
        ...
        <footer className="recruit-shell__footer">
          <div className="recruit-shell__footer-copy">
            <strong>
              Copyright &copy; {new Date().getFullYear()} <a href="http://cezannehr.com">Cezanne Recruitment</a>.
            </strong>{' '}
            All rights reserved.
          </div>
