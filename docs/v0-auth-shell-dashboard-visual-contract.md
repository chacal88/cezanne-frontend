...
| Auth token flows | Contract-reviewed | Current-app token state-hook screenshots captured; focused `/forgot-password`, `/reset-password`, `/confirm-registration`, register, and invite-token captures added | Partial | Register, confirm, reset, forgot, and invite-token hooks are implemented/tested; representative screenshots cover missing/invalid/expired/valid/success/failure/retry/pending-approval/bootstrap-failure. `/forgot-password` now follows the legacy direction for ready, submitting, success redirect, and top-right outcome flash placement; `/reset-password` now has focused legacy/current validating, invalid/expired, mismatch, submitting, success, and failure evidence; `/confirm-registration` now has focused invalid, valid-session, pending-approval, and bootstrap-failure handoff evidence; register public/token now has focused ready/validation/submitting/success/failure evidence; `/users/invite-token` now has focused current-only state evidence plus nonmatching legacy route probes. Legacy continuation details outside confirm-registration remain deferred. |
...
| Token flows | Promote current-app state map only | 45 deterministic screenshots across forgot/reset/confirm/register/invite-token for missing, invalid, expired, valid, success, failure, retry, pending approval, and bootstrap failure; focused `/forgot-password` same-run ready/submitting/outcome capture; focused `/reset-password` same-run validating/invalid/expired/mismatch/submitting/success/failure capture; focused `/confirm-registration` same-run invalid/valid-session/pending/bootstrap handoff capture; focused register public/token ready/validation/submitting/success/failure capture; focused invite-token current-only missing/ready/invalid/expired/failure/terminal/retry/pending/bootstrap capture; approved confirm-registration continuation decision | Backend token continuation payloads outside confirm-registration, invite semantics/field parity, final legacy parity where a matching legacy route exists; `/forgot-password` account-existence disclosure; final reset-token lifecycle enum; register/confirm/invite Figma evidence |
...
Only the explicit `/logout` handoff is replacement-approved by this package. Promotion for every other V0 sub-block means Figma/screen-flow drafting may start with the blockers above annotated.
