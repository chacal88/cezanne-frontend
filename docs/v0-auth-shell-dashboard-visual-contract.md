...
| Auth token flows | Contract-reviewed | Current-app token state-hook screenshots captured; focused `/forgot-password` same-run capture added | Partial | Register, confirm, reset, forgot, and invite-token hooks are implemented/tested; representative screenshots cover missing/invalid/expired/valid/success/failure/retry/pending-approval/bootstrap-failure. `/forgot-password` now follows the legacy direction for ready, submitting, success redirect, and top-right outcome flash placement. Legacy continuation details remain deferred. |
| SSO/callback/logout | Contract-reviewed | Current-app callback, missing-tenant, session-loss, and logout screenshots captured | Partial; `/logout` Pixel-parity-approved | Cezanne launch, Cezanne callback, SAML launch, SAML callback, missing tenant, missing code, exchange, failure, success, session-loss, and logged-out states are covered. `/logout` explicit handoff is approved after login-field parity recapture; provider popup behavior and backend payload details remain deferred. |
