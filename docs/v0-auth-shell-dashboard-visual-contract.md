...
|---|---|---|---|---|
| Auth entry `/` | Contract-reviewed | Primary login reference, current-app state-hook screenshots, and focused same-run login captures added | Partial | Login parity has source evidence; ready, filled, submitting, invalid, 2FA, SSO-required, activation/setup, bootstrap failure, and redirecting states are visually captured where feasible. Backend policy/copy, legacy bootstrap/redirecting capture, and final parity remain deferred. |
| Auth token flows | Contract-reviewed | Current-app token state-hook screenshots captured; focused `/forgot-password`, `/reset-password`, `/confirm-registration`, register, and invite-token captures added | Partial; `/forgot-password` Pixel-parity-approved | Register, confirm, reset, forgot, and invite-token hooks are implemented/tested; representative screenshots cover missing/invalid/expired/valid/success/failure/retry/pending-approval/bootstrap-failure. `/forgot-password` follows the legacy direction for ready, submitting, success redirect, and top-right outcome flash placement and is approved only by its focused evidence record; `/reset-password` now has focused legacy/current validating, invalid/expired, mismatch, submitting, success, and failure evidence; `/confirm-registration` now has focused invalid, valid-session, pending-approval, and bootstrap-failure handoff evidence; register public/token now has focused ready/validation/submitting/success/failure evidence; `/users/invite-token` now has focused current-only state evidence plus nonmatching legacy route probes. Legacy continuation details outside confirm-registration and approved `/forgot-password` states remain deferred. |
| SSO/callback/logout | Contract-reviewed | Current-app callback, missing-tenant, session-loss, and logout screenshots captured; focused provider callback and session-loss route captures added | Partial; `/logout` Pixel-parity-approved | Cezanne launch, Cezanne callback, SAML launch, SAML callback, missing tenant, missing code, exchange, failure, success, session-loss, and logged-out states are covered. `/logout` explicit handoff is approved after login-field parity recapture; `/session-lost` remains separate and unapproved; provider popup behavior and backend payload details remain deferred. |
...
|---|---|---|
| Forgot password | ready, submitting, sent, not-found/default failed, retry, return to login | Ready/submitting and outcome placement should follow the focused legacy-backed capture. Account-existence disclosure and backend enum confirmation remain approval blockers. |
...
| Invite token | missing/invalid/expired, ready/continuation, failed, terminal/return | Current-only state visuals are captured; exact continuation payload remains pending, and the located legacy `users.invite-token` route is an authenticated admin token-link modal rather than public invite acceptance. Do not invent invite payload fields. |
...
| Token flows | missing, invalid, expired, valid, success, failure, retry, pending approval, bootstrap failure across forgot-password, reset-password, confirm-registration, register, and invite-token hook families | Existing forgot/reset/register/invite captures plus 45 current-app token state hooks in `visual-evidence-assets/v0/v0-state-hooks-manifest.json`; focused `/forgot-password` same-run manifest in `visual-evidence-assets/v0/forgot-password-same-run-2026-04-23/capture-manifest.json`; focused reset-password records in `visual-evidence-assets/v0/auth-same-run-2026-04-23/capture-manifest.json`; focused confirm-registration records in `visual-evidence-assets/v0/confirm-registration-same-run-2026-04-23/capture-manifest.json`; focused register records in `visual-evidence-assets/v0/register-same-run-2026-04-23/capture-manifest.json`; focused invite-token current-only records in `visual-evidence-assets/v0/invite-token-same-run-2026-04-23/capture-manifest.json` | invite continuation payload/field parity, token backend schemas outside approved confirm-registration continuation and approved `/forgot-password` enum; final reset-token lifecycle enum; register/confirm/invite Figma evidence | `/forgot-password` only is Pixel-parity-approved |
| SSO/callback/session | Cezanne launch, missing tenant, Cezanne provider error, Cezanne missing code, Cezanne exchanging, Cezanne exchange failure, Cezanne bootstrap failure, Cezanne success, SAML launch, SAML provider error, SAML missing code, SAML exchanging, SAML exchange failure, SAML bootstrap failure, SAML success, explicit logout, session loss | Existing SAML/error/logout captures plus 15 current-app SSO/callback hooks and session-loss hook in `visual-evidence-assets/v0/v0-state-hooks-manifest.json`; focused provider callback same-run/reference manifest in `visual-evidence-assets/v0/provider-callback-same-run-2026-04-23/capture-manifest.json`; `/logout` post-handoff login recaptured after field parity fix; focused `/session-lost` current capture in `visual-evidence-assets/v0/auth-same-run-2026-04-23/capture-manifest.json` | provider popup framing, provider-specific error copy, callback exchange payload; `/session-lost` remains unapproved | `/logout` only is Pixel-parity-approved |
...
| Token flows | Promote current-app state map only; approve `/forgot-password` focused states | 45 deterministic screenshots across forgot/reset/confirm/register/invite-token for missing, invalid, expired, valid, success, failure, retry, pending approval, and bootstrap failure; focused `/forgot-password` same-run ready/submitting/outcome capture; focused `/reset-password` same-run validating/invalid/expired/mismatch/submitting/success/failure capture; focused `/confirm-registration` same-run invalid/valid-session/pending/bootstrap handoff capture; focused register public/token ready/validation/submitting/success/failure capture; focused invite-token current-only missing/ready/invalid/expired/failure/terminal/retry/pending/bootstrap capture; approved confirm-registration continuation decision | Backend token continuation payloads outside confirm-registration and approved `/forgot-password` enum, invite semantics/field parity, final legacy parity where a matching legacy route exists, final reset-token lifecycle enum, register/confirm/invite Figma evidence |
| Cezanne and SAML callback flows | Promote current-app transition map only | Launch/provider-error/missing-code/exchanging/exchange-failure/bootstrap-failure/success hooks for Cezanne, plus launch/callback hooks for SAML, with focused legacy/reference probes in `visual-evidence-assets/v0/provider-callback-same-run-2026-04-23/capture-manifest.json` | Provider popup framing, provider-specific payload/error copy, backend callback contracts, final legacy parity where a route-local legacy state exists |
| Logout and session loss | Promote covered states | `/logout` legacy-style handoff, current post-handoff login recapture, and separate `/session-lost` hook | `/logout` explicit handoff approved; final copy/policy parity for `/session-lost` remains separate and unapproved |
...
Only `/logout` and `/forgot-password` are replacement-approved, and only by their focused evidence records. Promotion for every other V0 sub-block means Figma/screen-flow drafting may start with the blockers above annotated.
