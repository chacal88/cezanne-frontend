...
| No-match/reset and safe action hooks | `visual-evidence-assets/v2/recapture-2026-04-22-parity-pass/new/07-database-search-no-match.png` through `new/20-detail-unhire-terminal-hook.png` | **Behaviour fixed/capture-ready; still blocked** | Database no-match/reset now has deterministic current-app capture. Upload success, review-request blocked, move-job parent-refresh, hire success, and unhire terminal are safely captureable through visible V2 hooks. Legacy side-by-side parity and broader denied/unavailable/stale/mobile state coverage remain required. |
