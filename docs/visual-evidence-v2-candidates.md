...
| No-match/reset and safe action hooks | `visual-evidence-assets/v2/recapture-2026-04-22-parity-pass/new/06-database-search-no-match.png` through `new/18-detail-move-parent-refresh-hook.png` | **Behaviour fixed/capture-ready; still blocked** | Database no-match/reset now has deterministic current-app capture. Upload success, review-request blocked, and move-job parent-refresh are safely captureable through visible V2 hooks. Legacy side-by-side parity and broader terminal/mobile/state coverage remain required. |
| Schedule launcher | `visual-evidence-assets/v2/legacy/legacy-v2-candidate-action-schedule-opened-seeded-1440x900.png` vs `visual-evidence-assets/v2/recapture-2026-04-22-parity-pass/new/20-action-schedule-modal-route.png` | **Improved; still blocked** | Greenfield no longer uses a standalone task page for this state; it now renders a route-owned modal wizard with Diego ApiSeed context, dark header, job details, interview fields, and radio groups. Exact wizard dimensions, stepper/header placement, copy, metadata, and footer behavior still differ. |
