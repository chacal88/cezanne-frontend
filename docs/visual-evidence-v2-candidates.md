...
| Schedule launcher | `visual-evidence-assets/v2/legacy/legacy-v2-candidate-action-schedule-opened-seeded-1440x900.png` vs `visual-evidence-assets/v2/current/greenfield-v2-candidate-action-schedule-ready-1440x900.png` | **Blocked** | Legacy opens a centered four-step modal over candidate detail; current greenfield uses a standalone task page. This is a material behaviour and composition mismatch. |
| Offer/reject launchers | Current greenfield action screenshots only; legacy offer/reject modal capture not yet confirmed | **Blocked** | Legacy offer/reject references were not captured successfully in this pass, so current standalone pages cannot be promoted as parity-ready. |
