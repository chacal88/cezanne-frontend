# V0-V5 Roadmap Review Closeout - 2026-04-22

## Purpose

This document records the review requested after V1 through V5 were implemented, with the later V0/V2/V5 parallel-thread consolidation appended. It verifies each roadmap slice against the active docs package, route metadata, runtime source, evidence logs, and validation commands.
...
|---|---|---|
| `npm test` | Passed | Consolidation run: 105 test files and 466 tests passed. |
| `npm run build` | Passed | Consolidation run: TypeScript build and Vite production build completed. Vite emitted only the existing large chunk warning. |
| `npm run smoke:r0:ui` | Passed | Consolidation run: 28 Playwright smoke tests passed. |
| `npm run smoke:r0:http` | Passed | Consolidation run: HTTP-level route smoke passed; script rebuilt the app and emitted only the existing large chunk warning. |
...
| Visual evidence | `docs/visual-evidence-v0-auth-shell-dashboard.md`, `docs/visual-evidence-v1-jobs.md`, `docs/visual-evidence-v2-candidates.md`, `docs/visual-evidence-v3-public-external-token.md`, `docs/visual-evidence-v4-operations.md`, `docs/visual-evidence-v5-sysadmin-platform.md` |

## Release review matrix

| Release | Roadmap scope reviewed | Runtime/doc verification | Current audit decision |
|---|---|---|---|
| V0 / R0 Auth, shell, dashboard | Auth entry, token flows, SSO/Cezanne/SAML, logout/session-loss, shell account profiles, dashboard, notifications, inbox | V0 routes, metadata, public auth pages, account settings states, and R0 smoke coverage are implemented. Current-app state-hook screenshots cover login non-happy states, token states, SSO/callback states, explicit logout/session-loss separation, and user/company/agency profile states. | Current-app evidence coverage improved and covered sub-blocks may feed Figma drafting with backend/API annotations. Not replacement-approved: backend auth/token/provider payloads, live notification/inbox transport, dashboard aggregate/calendar parity, shell/account parity, and final legacy/current/Figma pixel parity remain blockers. |
...
| V5 / R5 Platform + long-tail | SysAdmin users, favorite-request queue, master data, company subscription, taxonomy, requisition workflows, API endpoints, forms/download, settings leftovers, token reconciliation | R5 route families are registered and metadata marks them implemented. V5 evidence captures all currently reachable SysAdmin/platform route shells and the non-SysAdmin fallback boundary. Runtime fixture hooks and tests now cover platform users, favorite requests, master data, company subscription, and taxonomy depth states. | Closed for foundation route/runtime coverage. Not Figma-ready: screenshot capture of the new hooks, backend/API schemas, responsive/mobile omission decisions, dashboard copy polish, and pixel-parity review where legacy screens exist remain blockers. |

## Item-by-item status

### V0 Auth, Shell, Dashboard

| Item | Status | Evidence |
|---|---|---|
| Auth entry and token flows | Current-app evidence captured; not replacement-approved | `visual-evidence-v0-auth-shell-dashboard.md`, V0 state-hook manifest, public auth tests. Backend 2FA/activation/setup/token continuation contracts remain deferred. |
| SSO/Cezanne/SAML and session transitions | Current-app evidence captured; not replacement-approved | `/auth/cezanne`, callback, SAML, logout, and session-loss tests/screenshots. Provider callback payloads and popup treatment remain deferred. |
| Shell account profiles | Current-app evidence captured; not replacement-approved | User, hiring-company, and recruitment-agency dirty/saving/saved/save-failed/retry/degraded/denied hooks and account settings tests. Persistence API and account-menu parity remain deferred. |
| Dashboard, notifications, inbox | Covered for V0 drafting inputs; not replacement-approved | Dashboard legacy/current captures, fixture-backed notification/inbox captures, and R0 UI smoke. Live notification API, live inbox send/provider-blocked states, and dashboard aggregate/calendar parity remain deferred. |
...
| V0 backend/live API/parity | Confirm backend/API contracts for auth/token/provider payloads, live notification and inbox transport, dashboard aggregate/calendar parity, and final shell/account visual parity before replacement approval. |
| V5 platform visual depth | Capture the new fixture/test-hook states or record product-approved omissions for non-ready, mutation, terminal, stale, not-found, and permission-denied states. |
...
V0 through V5 are implemented and documented at the route, metadata, capability, and tested runtime foundation level. V0 covered sub-blocks, V1, V3, and V4 are ready for Figma screen-flow/base-frame drafting with deferred backend/provider/schema annotations. V2 remains behavior-covered but visual-parity blocked. V5 remains foundation-complete with runtime hook depth, but not Figma-ready until platform state screenshots are captured or a product-approved omission is recorded. No legacy-backed route is production replacement-approved until pixel parity is confirmed or explicitly excepted by product.
