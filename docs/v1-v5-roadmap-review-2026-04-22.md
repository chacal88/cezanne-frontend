...
| V5 / R5 Platform + long-tail | SysAdmin users, favorite-request queue, master data, company subscription, taxonomy, requisition workflows, API endpoints, forms/download, settings leftovers, token reconciliation | R5 route families are registered and metadata marks them implemented. V5 evidence captures all currently reachable SysAdmin/platform route shells, the non-SysAdmin fallback boundary, and 124 desktop state-hook screenshots for platform users, favorite requests, master data, company subscription, and taxonomy depth states. | Closed for desktop current-app screen-flow Figma handoff. Final replacement remains blocked by backend/API schemas, responsive/mobile scope decisions, dashboard copy polish, and pixel-parity review where legacy screens exist. |
...
V0 through V5 are implemented and documented at the route, metadata, capability, and tested runtime foundation level. V0 covered sub-blocks, V1, V3, V4, and V5 are ready for Figma screen-flow/base-frame drafting with deferred backend/provider/schema annotations. V2 remains behavior-covered but visual-parity blocked. No legacy-backed route is production replacement-approved until pixel parity is confirmed or explicitly excepted by product.
