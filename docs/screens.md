...
| `/parameters/:settings_id?/:section?/:subsection?` | PageWithStatefulUrl | settings | settings-container | HC User, HC Admin, RA User, RA Admin | `canEnterSettings` | `hc` or `ra`; subsection entitlements vary | Compatibility route only: resolves registered operational settings subsections, preserves subsection-specific deny/fallback behavior, and does not become a monolithic settings page. | M | R4-R5 |
