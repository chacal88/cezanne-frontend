...
This inventory supports archived OpenSpec change `auth-replacement-readiness-package` and active contract closeout `v0-auth-token-session-contract-closeout`.
...
| `/` 2FA required/failed | Same as `/` | Legacy 2FA behavior exists in login template/controller source; `ms-auth` sends a six-digit email code valid for 10 minutes and marks verified codes as used | Current deterministic hooks exist | `legacy-backed`, `backend-blocked` | Structured envelope names, explicit resend/lockout policy, optional remember-device behavior, and final copy |
...
| `/forgot-password` `mail_not_found` | Same as `/forgot-password` | Legacy shows error toastr for account-not-found | Current adapter maps to failed message | `legacy-backed` | Contract closeout preserves legacy disclosure until Product/Security opens a separate change; Figma/parity evidence still blocks replacement |
...
| `/confirm-registration/:token` invalid/expired | `Public/Token`, auth registration, `canUseAuthTokenFlow` | Legacy route resolves invalid token with toastr and home redirect | Current invalid screenshot exists | `legacy-backed` | Decision approved 2026-04-23: return to `/` with safe error copy and no session; same-run legacy/current capture and Figma frame still required |
| `/confirm-registration/:token` valid/session-ready | Same as confirm-registration | Legacy token-valid plus user/token redirects dashboard | Current route can bootstrap session | `legacy-backed` | Decision approved 2026-04-23: `token_valid` plus session token bootstraps and redirects to `/dashboard`; same-run dashboard/login landing evidence still required |
| `/confirm-registration/:token` pending approval/bootstrap failure | Same as confirm-registration | Legacy token-valid without user opens approval modal on home | Current deterministic hooks exist | `legacy-backed` | Decision approved 2026-04-23: `approval_pending` or `token_valid` without token returns to `/` with approval-pending copy; `bootstrap_failed` returns to `/` with safe login-retry copy; modal/copy parity and Figma evidence still required |
...
| `/users/invite-token` missing/invalid/expired | `Public/Token`, auth invite, `canUseAuthTokenFlow` | 2026-04-23 source check found legacy `users.invite-token` as an authenticated admin modal under `/users`, used to copy an organization token link; no matching public invite acceptance route confirmed | Current route, hooks, and focused current-only capture exist | `current-app-only` | Contract closeout keeps this route current-app-only for now; replacement requires future product/backend decision plus Figma/parity evidence |
| `/users/invite-token` ready/continuation/failure/terminal | Same as invite-token | Legacy continuation visual not confirmed; legacy public-path probes redirect to `/` and are reference-only | Current hooks and focused current-only capture exist | `current-app-only` | Contract closeout keeps this route current-app-only for now; exact continuation fields and terminal behavior still require backend schema before production use |
| `/auth/cezanne/:tenantGuid?` launch/missing tenant | `Public/Token`, auth cezanne, `canCompleteSsoCallback` | Legacy launch route exists for `/auth/cezanne/:tenantGuid`; no-tenant route is current explicit safety state | Current launch and missing-tenant hooks plus focused provider callback capture exist | `legacy-backed` for launch, `current-app-only` for missing tenant, `backend-blocked` | Provider launch framing and missing-tenant copy |
| `/auth/cezanne/callback` missing code/provider error/exchanging/exchange failure/bootstrap failure/success | `Public/Token`, auth cezanne, `canCompleteSsoCallback` | Legacy callback route exchanges code, stores failed cookie, redirects home/dashboard; focused captures show resolver/redirect probes rather than route-local callback UI | Current callback states, hooks, and focused provider callback capture exist | `legacy-backed`, `backend-blocked` | Error taxonomy, cookie/modal parity, callback payload, success redirect, Figma parity treatment for resolver-only legacy states |
| `/auth/saml` launch/provider error/missing code/exchanging/exchange failure/bootstrap failure/success | `Public/Token`, auth sso, `canCompleteSsoCallback` | Legacy `/auth/saml?code&error` route and launch form exist; focused captures show launch template plus resolver/modal behavior | Current launch/callback states, hooks, and focused provider callback capture exist | `legacy-backed`, `backend-blocked` | Error/modal parity, profile email launch behavior, callback payload |
...
| `/session-lost` | Shell/session `Page`, `canStartSession` | Legacy session-loss-specific visible state is not confirmed; legacy `old_token` warning is policy evidence only | Current visible route/hook exists | `current-app-only`, `backend-blocked` | Contract closeout confirms safe copy/local clearing, but trigger taxonomy and Figma evidence still block replacement |
...
| 2FA policy | `ms-auth` initiates 2FA by publishing a six-digit email verification code, stores codes with 10-minute expiry, verifies by user/remote address/code, and marks verified codes as used. Current frontend keeps failed verification in the 2FA step. | No source-backed resend endpoint or lockout policy was located; Backend/Auth should confirm whether `validForThirtyDays` creates a frontend-visible remember-device state. |
| Forgot-password disclosure | Legacy `/forgot-password` consumes `mail_not_found`, `mail_sent`, and `mail_error`. `mail_not_found` is an error toast, `mail_sent` is a success toast followed by home redirect, and `mail_error` is a generic error toast. Contract closeout preserves this disclosure until Product/Security opens a deliberate change. | Replacement still needs matched Figma/parity evidence and stable backend enum proof. |
...
| Register and invite continuation | Legacy `/register/:token` treats the token as optional/squashed. Invitation token validation returns `token_invalid` or `token_valid`; valid invite can prefill user/company fields and submission returns home with success modal. 2026-04-23 source check found legacy `users.invite-token` only as an authenticated admin token-link modal, not public invite acceptance. Contract closeout keeps `/users/invite-token` current-app-only for now, not an alias. | Backend/Auth should confirm invite payload fields and invalid/expired split before production use; any alias/canonical change needs a future product/backend change. |
...
- Backend/product confirmation is still required for structured auth response envelopes, reset/registration/invite payload fields, provider callback taxonomy, 2FA resend/lockout/remember-device semantics, activation/setup reason payloads, and session-loss trigger taxonomy.
