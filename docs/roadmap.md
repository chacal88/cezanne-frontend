...
V0 consolidation update (2026-04-22; focused approvals updated 2026-04-23): auth entry, token flows, SSO/Cezanne/SAML callbacks, logout/session-loss separation, shell account profile states, dashboard, notifications, and inbox now have current-app runtime/evidence coverage for the documented local state hooks. Backend/API contracts for 2FA policy, activation/setup reasons, token continuation payloads outside the focused `/forgot-password` enum, provider callback payloads, live notifications/inbox transport, and dashboard aggregate/calendar parity remain deferred. `/logout` and `/forgot-password` are production replacement-approved only by their focused evidence records; no other V0 surface is production replacement-approved.

V1-V5 review update (2026-04-22; V5 state-hook evidence updated 2026-04-23): V1, V3, V4, and V5 are closed for current screen-flow handoff bases; V5 is desktop current-app Figma-ready after 124 state-hook screenshots for non-ready/mutation/depth states, with backend schemas and replacement parity still deferred; V2 is behaviour-covered and still not final parity-cleared, but the original structural blocker has now been reduced to a polish-level residual concentrated in the candidate database ready frame plus smaller detail/action refinements. Product direction for V2 Candidates is full parity: no database/detail/action-flow deviation is accepted unless a future product decision explicitly records the exception and gap id. The parity passes have improved V2 database density/saved lists, candidate action modal composition, candidate detail legacy modal entry points, recapture seed parity for Finn/Diego/API-seed-style evidence, schedule/reject/email editor-style action composition, database ready-frame vertical rhythm, search sizing, fuller table/pagination coverage, database menu/header icon treatment, detail card/tab vertical proportions, schedule/reject modal geometry, and Email tab composer parity, with `npm test`, `npm run build`, and `npm run smoke:r0:ui` green.
...
| Visual evidence capture | In progress | V0-V5 visual contracts + `visual-evidence-capture-plan.md` + V0/V1/V2/V3/V4/V5 evidence logs | V0, V1, V3, V4, and V5 screen-flow bases are captured and unblocked for Figma drafting with deferred backend/provider/schema annotations, but still require route-specific pixel-parity signoff before replacement unless a focused approval record says otherwise. V0 `/logout` and `/forgot-password` are the current focused approvals; other token/SSO/session/account profile hook screenshots are current-app evidence, not backend/API or replacement approval. V2 candidate rows are behaviour-captured and parity-blocked. V5 has first-pass current-app evidence plus state-hook screenshots and is desktop current-app Figma-ready while backend schemas, responsive/mobile scope, and pixel-parity review remain deferred. |
...
Planning rule: do not reopen provider readiness gates as the current implementation package unless a new provider family or operational consumer is intentionally added. The active phase is visual evidence capture against the V0-V5 visual-readiness contracts. Figma production is partially unblocked only for rows/sub-blocks promoted to `Figma-ready` by evidence, but `Figma-ready` is a screen-flow drafting status, not replacement signoff. Every legacy-backed row remains subject to side-by-side pixel-parity approval before launch replacement unless a focused route evidence record explicitly approves it; current exceptions are `/logout` and `/forgot-password`. V3 public/external and integration token rows, V4 operations rows, and V5 SysAdmin/platform desktop current-app rows are promoted for screen-flow/base-frame Figma handoff with deferred backend/provider/schema annotations.
...
- V5 platform/long-tail is foundation-complete in route/runtime coverage and desktop current-app Figma-ready for covered SysAdmin/platform rows after fixture-hook screenshot capture for platform user, favorite request, master-data, company subscription, and taxonomy depth states. Backend schemas, responsive/mobile scope, and replacement pixel parity remain deferred.
