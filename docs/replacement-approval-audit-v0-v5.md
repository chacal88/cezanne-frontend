...
| Token flows `/forgot-password`, `/reset-password/:token`, `/confirm-registration/:token`, `/register`, `/register/:token`, `/users/invite-token` | Yes for current-app state map | Partial | No | Partial | TBD | No | Focused `/forgot-password`, `/reset-password`, `/confirm-registration`, register, and invite-token evidence now exists, and `/confirm-registration` continuation decision is approved. `/users/invite-token` evidence is current-only because the located legacy route is an authenticated admin token-link modal, not public invite acceptance. Remaining blockers include token lifecycle enums outside the approved confirm flow, continuation payload field parity, pending/used/expired semantics, register/confirm/invite Figma evidence, matched Figma parity, and broader token-flow legacy parity where a matching legacy route exists. |
...
V0 auth contract closeout update (2026-04-23): `v0-auth-token-session-contract-closeout` narrows several backend blockers with source-backed frontend contract decisions, including email-delivered 10-minute 2FA codes, no located source-backed 2FA resend/lockout endpoint, preserved forgot-password disclosure, current-app-only `/users/invite-token`, SAML short-lived callback code exchange, and safe current-app-only `/session-lost` copy/local clearing. This does not change replacement approval: all V0 auth rows except explicit `/logout` remain unapproved until structured backend envelopes, Figma evidence, and matched parity are complete.
